...
class PyFastbssTestbed:
    ...
    def wavs_to_matrix_S(self, folder_address, duration, source_number):
        ...
        for i in range(source_number):
            _s, sample_rate = librosa.load(
                wav_filenames[random_indexs[i]], sr=None, mono=True)
            wav_length = np.shape(_s)[-1]
            wav_range = int(duration*sample_rate)
            if wav_range > wav_length:
                raise ValueError('Error - wav_to_S : The wav_range too big !')
    ...
    def wavs_to_matrix_S_by_folders(self, folder_addrs, duration):
        source_number = len(folder_addrs)
        S = []
        for i in range(source_number):
            wav_path = folder_addrs[i] + '/*.wav'
            wav_filenames = glob.glob(wav_path)   
            random_index = np.random.randint(len(wav_filenames))
            _s, sample_rate = librosa.load(
                wav_filenames[random_index], sr=None, mono=True)
            wav_length = np.shape(_s)[-1]
            wav_range = int(duration*sample_rate)
            if wav_range > wav_length:
                raise ValueError('Error - wav_to_S : The wav_range too big !')
    ...
    def generate_matrix_S_A_X_by_folders(self, folder_addrs, duration, mixing_type="random", max_min=(1, 0.01), mu_sigma=(0, 1)):
        S = self.wavs_to_matrix_S_by_folders(folder_addrs, duration)
