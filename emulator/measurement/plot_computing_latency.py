...
if __name__ == '__main__':
    ...
    tc_cf_conf = get_conf_interval(
        number_node, computing_latency_cf, conf_rate)
    tc_cf_hbh_conf = get_conf_interval(
        number_node, computing_latency_cf_hbh, conf_rate)
    tc_sf_conf = get_conf_interval(
        number_node, computing_latency_sf, conf_rate)
    ...
    with plt.style.context(['science', 'ieee']):
        ...
        line1_fill = ax.fill_between(x_index, tc_server_cf_conf[:, 1],
        ...
        line2_fill = ax.fill_between(x_index, tc_server_sf_conf[:, 1],
        ...
        line3_fill = ax.fill_between(x_index, tc_server_cf_hbh_conf[:, 1],
        ...
        plt.savefig('./emulator/measurement/plot/computing_latency_server_emu.pdf',
        ...
        line1_fill = ax.fill_between(x_index, tc_cf_conf[:, 1],
                                     tc_cf_conf[:, 3], color=colordict['compute_forward'], alpha=0.2)
        line2 = ax.errorbar(
            x_index, tc_sf_conf[:, 2], color=colordict['store_forward'], lw=1, ls='-', marker=markerdict['store_forward'], ms=5)
        line2_fill = ax.fill_between(x_index, tc_sf_conf[:, 1],
                                     tc_sf_conf[:, 3], color=colordict['store_forward'], alpha=0.2)
        line3 = ax.errorbar(
            x_index, tc_cf_hbh_conf[:, 2], color=colordict['store_forward_ia'], lw=1, ls='-', marker=markerdict['store_forward_ia'], ms=5)
        line3_fill = ax.fill_between(x_index, tc_cf_hbh_conf[:, 1],
                                     tc_cf_hbh_conf[:, 3], color=colordict['store_forward_ia'], alpha=0.2)
        ax.set_xlabel(r'Number of nodes $k$')
        ax.set_ylabel(r'Computing latency ($s$)')
        # ax.set_xlim([-0.2, 4.2])
        ax.set_yticks(np.arange(0, 7, 1))
